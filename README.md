...
stored on the end-user's computer. Each database has a unique name and a version, which
is like a revision.
...
If you need more fine-grained control over versions and the upgrade process you
can also specify a version number and a single upgrade function:
...
app then your new code may have changed to use a newer version of the database.
...
`outdated` promises on your pdb instance to handle it:
...
More info
---------
The following resources give more info on working with IndexedDB in general.
PromisedDB changes the event model but the core principles of stores, indexes,
cursors, keys and transasctions is shared.

* [API reference on MDN](https://developer.mozilla.org/en-US/docs/Web/API/IndexedDB_API)
* [Using IndexedDB on MDN (somewhat outdated)](https://developer.mozilla.org/en-US/docs/Web/API/IndexedDB_API/Using_IndexedDB)
